...
import numpy, time, sys, random, os, pylab, expclass, pickle, readit3x3
...
for (circle, loc) in zip(circleList, StimLocs):
    circle.setPos(loc)
for (line, loc) in zip(lineList, StimLocs):
    line.setPos(loc)
...
def executeExample():
    ...
    for prompt in INST:
        ...
        if index == len(INST)-3:
            ...
            probe([0,0,0], 'Line', [0,1,2], trialExample)
        index+=1
...
def main():
    ...
    practice = 'False'
    blockNumber = 1
    trialsInBlock = 40   # For real experiment // should be 40

    shape = visual.Rect(win, lineWidth = 0, fillColor = 'yellow', fillColorSpace = 'rgb255', pos = (240,60), size = (230,100))
    screenPrompt("You will be tested on one set of memory items\n(press space to start)", shape)
    executeBlock("Single", blockNumber, 20, practice)
